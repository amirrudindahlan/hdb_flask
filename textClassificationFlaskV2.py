...
import sys
sys.path = [r'C:\Users\HuaSheng\Anaconda3\envs\tfkrjn\Lib\site-packages']+[''] + sys.path
...
def make_predict():
    ...
    with tf.device('/cpu:0'):
        financeMdl = load_model('./textClassWithActions.h5')
    financeMdl.summary()
    data = request.get_json(force=True)
    textIn = [data['dataIn']]
    print(textIn)

    textIn = re.sub(r"[^A-Za-z0-9()!?\'\`%$]", " ", textIn[0]) # keep also %$ but removed comma
    ...
    + inv_map1[np.argmax(y_hat[1])] + ' department. Proposed Response: ' + inv_map3[np.argmax(y_hat[3])]  
